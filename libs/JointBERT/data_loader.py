...
from utils import get_intent_labels, get_slot_by2task_labels, get_slot_labels
...
class JointProcessorSlotBy2task(JointProcessor):    
    """Processor for the JointBERT data set """

    def __init__(self, args):
        self.args = args
        self.intent_labels = get_intent_labels(args)
        self.org_slot_labels = get_slot_labels(args)
        if "slotby2task" in self.args.task:
            self.slot_labels, self.slot_type_labels, self.org_slot_labels = get_slot_by2task_labels(args) 
        else:
            self.slot_labels, self.slot_type_labels, self.org_slot_labels = self.org_slot_labels, [], self.org_slot_labels
